                ...
                with patch('app.DB.session.commit',
                    ...
                    with patch('models.Person.query') as mocked_query:
                        ...
                        for i in range(len(actual_result)):
                            self.assertEqual(actual_result[i].username,
                                             expected_result[i].username)
                            self.assertEqual(actual_result[i].userscore,
                                             expected_result[i].userscore)
                        # print("\n")
...
class add_user_test_fail(unittest.TestCase):
    def setUp(self):
        self.success_test_params = [
            {
                KEY_INPUT: "bobby",
                KEY_OUTPUT: [models.Person(username="kevin", userscore=400)],
            },
            {
                KEY_INPUT:
                "jimmy",
                ...
                with patch('app.DB.session.commit',
                    ...
                    with patch('models.Person.query') as mocked_query:
                        ...
                        if (len(actual_result) < len(expected_result)):
                            for i in range(len(actual_result)):
                                self.assertNotEqual(
                                    actual_result[i].username,
                                    expected_result[i].username)
                                self.assertNotEqual(
                                    actual_result[i].userscore,
                                    expected_result[i].userscore)
                        if (len(actual_result) > len(expected_result)):
